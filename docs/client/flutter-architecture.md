...
- 统计。
- 设置。
- 基础资料管理页：从设置页进入，可新增、编辑、删除和调整分类、使用人、账户显示顺序；账户按银行名称和可选银行卡尾号维护；删除调用服务端软删除 API；分类、使用人和账户均通过三横线拖拽手柄长按调整顺序，顺序通过服务端 `sort_order` 持久化并在 Web/Android 一致生效。
...
短信导入页面只在服务端 bootstrap 返回 `features.sms=true` 时进入导航。Web 设备的该开关固定为 false；Android 设备进入页面后仍需用户主动授权并点击“重新扫描”才会读取历史短信。导入前可按解析后的交易日期和银行名称过滤候选；银行名称来自已创建账户。Android 历史扫描查询本机 `content://sms`，以覆盖不同厂商对 inbox/all 视图的差异；后台广播只进入进程内候选队列，不写本地离线队列。页面 AppBar 在刷新按钮左侧提供“清除本机短信隐藏记录”操作，仅删除 Android 本机 `sms_imported_hashes_v1` 缓存，方便重新扫描此前被隐藏的候选；该操作不删除服务端交易、本机模板或设备 token，且不在其他页面展示。若扫描后没有候选，页面只显示读取行数、95588 行数、正文非空行数、模板匹配数、候选数和隐藏数等本机诊断计数，不显示、不上传短信正文。
...
- 交易列表筛选区在交易方向/分类、使用人/账户两行中按 `2:4` 宽度比例排列，起始日期和结束日期各占一行；交易日期筛选显示为 `MM-DD-YYYY`。
- 交易列表记录内第一排显示分类，第二排显示详细描述和交易对象，不显示账户信息，第三排显示使用人和不含日期的交易时间。
