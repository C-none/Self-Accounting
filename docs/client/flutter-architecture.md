...
- 统计。
- 设置。
- 基础资料管理页：从设置页进入，可新增、编辑和删除分类、使用人、账户；账户按银行名称和可选银行卡尾号维护；删除调用服务端软删除 API。

设置页同时提供当前服务地址修改、当前设备名修改和最近审计日志查询。服务地址在卡片内按 IP/主机和端口输入并保存到本机安全存储，保存后后续请求使用新地址，不在编辑控件退出时触发 bootstrap 刷新；设备名修改调用当前设备接口并刷新 bootstrap；日志查询仅 admin 设备显示，列表默认展示时间、动作、实体和设备名，可展开单条日志查看日志 ID、实体 ID、设备 ID、原始动作和原始实体类型，不展示审计 payload、token、配对码或短信正文。
...
短信导入页面只在服务端 bootstrap 返回 `features.sms=true` 时进入导航。Web 设备的该开关固定为 false；Android 设备进入页面后仍需用户主动授权并点击“重新扫描”才会读取历史短信。导入前可按解析后的交易日期和银行名称过滤候选；银行名称来自已创建账户。后台广播只进入进程内候选队列，不写本地离线队列。
